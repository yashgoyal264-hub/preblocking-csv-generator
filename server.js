...
// Custom price validation and formatting function
function validateAndFormatCustomPrice(priceStr, context) {
    if (!priceStr || priceStr.trim() === '' || priceStr === '#N/A' || priceStr === '#DIV' || priceStr.toLowerCase() === 'na') {
        return '2500';
    }
    ...
    if (isNaN(numPrice)) {
        console.log(`Invalid custom_price "${cleanPrice}" in ${context}, setting to 2500`);
        return '2500';
    }
    ...
    if (numPrice !== intPrice) {
        console.log(`Converted decimal custom_price ${numPrice} to integer ${intPrice} in ${context}`);
    }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const parsedRows = rows.map((line, index) => {
                ...
                // Validate and normalize custom_price (column 14)
                if (columns.length > 14) {
                    const originalCustomPrice = columns[14];
                    columns[14] = validateAndFormatCustomPrice(originalCustomPrice, `Row ${index + 2}, custom_price`);
                }
